...
def top_vulnerabilities(df, iso3, n=3):
    ...
    # keep only vulnerability indicators that exist in df
    indicators = [c for c in VULN_INDICATORS if c in df.columns]

    # normalize each indicator (0–1) across all countries
    norm_df = df.copy()
    ...
    # get latest year row for selected country
    ...
    # sort descending: higher normalized value = higher vulnerability
